...
JPATH="$(cd "$(dirname "$0")" && pwd)/java/bin"
...
exitclean() {
    tput cnorm
    kill "$FAILCOUNTD_PID" 2>/dev/null
    wait "$FAILCOUNTD_PID" 2>/dev/null
    exit
}
trap 'exitclean' EXIT INT TERM

owner_count() {
    jq '[.users[] | select(.permission=="owner")] | length' "$CONFIG_FILE" || printf 0
}
...
#   selected_theme="$(java -cp $JPATH themefind "$(jq -r '.selected_theme' "$CONFIG_FILE")")"
#   unselected_theme="$(java -cp $JPATH themefind "$(jq -r '.unselected_theme' "$CONFIG_FILE")")"
#   main_theme="$(java -cp $JPATH themefind "$(jq -r '.main_theme' "$CONFIG_FILE")")"
...
main_menu() {
    while true; do
    menu "Login" "Enter Shell" "Manage Users" "Settings" "Advanced Options" # "Quit" # uncomment for quit option. not recommended as it can somtimes cause users to allow the shell without authenticating. only used for easier debugging
        case "$selected" in
...
manage_users() {
	...
	"Add user")
	    ...
	    if [ "$new_password" = "$new_confirm_pass" ]; then
			...
			jq --arg id "$new_id" --arg pass "$pass" --arg salt "$salt" --arg perm "$new_permission" --arg regdate "$(date)" --argjson failcount 0 --argjson timeleft 0 '.users += [{"id": $id, "password": $pass, "salt": $salt, "permission": $perm, "regdate": $regdate, "failcount": $failcount, "timeleft": $timeleft}]' "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
			reload
			clear
    ...
    "My Info")
        if [ -z "$CURRENT_USER" ]; then
            ...
            while true; do
                log "Personal Info accessed by $CURRENT_USER"
		...
		case "$selected" in
		    "Manually edit config.json(unsafe)")
                if [ "$CURRENT_PERM" = "owner" ]; then
                    ...
                    log "$CURRENT_USER manually edited the config.json"
                else
...
run_login() {
    while true; do
        ...
        failcount="$(jq -r --arg id "$selected_user" '.users[] | select(.id==$id) | .failcount // 0' "$CONFIG_FILE")"
        if [ $failcount -ge 5 ]; then
            ShowError 3 "Too many failed login for this account.\nTo try again you may leave the menu open for 5 minutes without trying to login with this account"
            return
        fi
	    printf "\n%sEnter password for %s%b" "$main_theme" "$selected_user" "\033[0m"
        readsilp inputpass
        printf "%b" "\n"
        local salt="$(jq -r --arg id "$selected_user" '.users[] | select(.id==$id) | .salt' "$CONFIG_FILE")"
	    ...
	    if [ ! "$inputpass" = "$CURRENT_PASS" ]; then
            ((failcount++))
            jq --arg id "$selected_user" --argjson failcount $failcount '.users |= map(if .id==$id then .failcount=$failcount else . end)' "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
            [ $failcount = 5 ] && jq --arg id "$selected_user" --argjson timeleft 300 '.users |= map(if .id==$id then .timeleft=$timeleft else . end)' "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
            unset failcount
            ...
            # ADD GOCRYPTFS OR SIMILAR IN THE FUTURE?
...
quitexec() {
    exec "$BOOTSHELL" || exec /bin/env bash || exec bash || exec sh
}

failcountd() {
    log "Failcountd started"
    while [ "$AccountLockEnabled" = true ]; do
        sleep 5
        local locked_users="$(jq -r '.users[] | select(.failcount >= 5) | .id' "$CONFIG_FILE")"
        for lockeduser in ${locked_users[@]}; do
            local timeleft="$(jq -r --arg id "$lockeduser" '.users[] | select(.id==$id) | (.timeleft // 0)' "$CONFIG_FILE")"
            ...
            if [ "$timeleft" -le 0 ]; then
                jq --arg id "$lockeduser" --argjson failcount 0 '.users |= map(if .id==$id then .failcount=$failcount else . end)' "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
                [ "$timeleft" -lt 0 ] && timeleft=0
                continue
            fi
            jq --arg id "$lockeduser" --argjson newtimeleft $newtimeleft '.users |= map(if .id==$id then .timeleft=$newtimeleft else . end)' "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
        done
    done
}
welcmesg() {
...
reset_users() {
    prompt "Are you sure? this will remove all users. [y/*]" askreset_users
    [ "$askreset_users" = "y" ] && jq '.users = []' "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
    log "Users reset by ${CURRENT_USER:-Unknown}"
}

enter_shell() {
    if [ "$CURRENT_PERM" = "member" ] || [ "$CURRENT_PERM" = "owner" ]; then
        clear
        log "Shell accessed by $CURRENT_USER"
        welcmesg
        tput cnorm
        exec "$BOOTSHELL"
    else
...
advanced_options() {
    while true; do 
        clear
        menu "Temporary Shell" "Reload Configs" "View Logs" "Reset Users" "Back"
        case "$selected" in
            "Temporary Shell")
                menu "sh" "dash" "bash" "zsh" "ksh" "csh" "fish" "Back"
                clear
                [ "$selected" = "Back" ] && return
                if command -v "$selected" >/dev/null 2>&1; then
                    ...
                    ShowError 1 "Selected shell is not installed or accessible please install it"
                    return
                fi
                while true; do
            ...
            "View Logs") less "$LOGFILE" ;;
            "Back") clear; return ;;
        esac
    done
}
failcountd >/dev/null 2>&1 &
FAILCOUNTD_PID=$!
main_menu || log "Main menu failed to start!"